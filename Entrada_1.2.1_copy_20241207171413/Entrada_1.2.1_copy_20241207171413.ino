...
int doorflag;
int numtry;
int TIME_TO_OPEN = 2000;  // time to door to be opened(ms)
...
void setup() {
  ...
  wg.begin(17,18,Reader1D0Interrupt,Reader1D1Interrupt);
	wg2.begin(15,4,Reader2D0Interrupt,Reader2D1Interrupt);
...
void rereading() {
  ...
  while (EEPROM.get(numint1, RFID_message) != 4294967295) {
...
void loop() {
  ...
  if (wg2.available()) {
    ...
    Serial.println(wg2.getCode());
    lastcard = wg2.getCode();
